@php
    ...
    foreach ($faqElements as $faqElement) {
        $faqJson['mainEntity'][] = [
            '@type' => 'Question',
            'name' => __(@$faqElement->data_values->question),
            'acceptedAnswer' => [
                '@type' => 'Answer',
                'text' => __(@$faqElement->data_values->answer),
            ],
        ];
    }
@endphp
...
@if (!empty($faqJson['mainEntity']))
    @push('structured_data')
        <script type="application/ld+json">
            {!! json_encode($faqJson, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT) !!}
        </script>
    @endpush
@endif
