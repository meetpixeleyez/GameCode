...
class HttpClient
{
    ...
    public function execute(HttpRequest $httpRequest)
    {
        ...
        if (strpos($this->environment->baseUrl(), "https://") === 0) {
            // In local/dev environments with self-signed certs, disable strict SSL verification
            if (function_exists('app') && (app()->environment('local') || (function_exists('config') && config('app.debug')))) {
